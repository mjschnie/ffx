...
public class CcmaConstraint implements Constraint {
     ...
     * are set to zero to make K-1 sparse and thus keep CCMA tractable.
     * <p>
     * Due to leaking-this issues, a thin Factory method is provided to build this.
     * <p>
     * The constrainedAngles will constrain both component bonds and the triangle-closing distance.
     * The constrainedBonds list should not include any bonds included in the constrained angles.
     *
    ...
    private CcmaConstraint(List<Bond> constrainedBonds, List<Angle> constrainedAngles, final Atom[] allAtoms, final double[] masses, double nonzeroCutoff) {
        ...
        uniqueIndices = IntStream.concat(Arrays.stream(atoms1), Arrays.stream(atoms2)).
                sorted().
                distinct().
                toArray();
        ...
        for (int i = 0; i < nConstraints; i++) {
            ...
            for (int j : coupledConstraints) {
                ...
                if (atomi0 == atomj0) {
                    ...
                    assert atomi0 != atomj1; // Yes IntelliJ, it should always be true. That's why I'm asserting it.
                    atoma = atomi0;
                    atomb = atomi1;
                    atomc = atomj1;
                ...
                if (!foundAngle) {
                    logger.severe(String.format(" Could not find the angle between coupled constraints %d, %d", i, j));
                }
            }
        }
